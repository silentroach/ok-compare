  ...
  const body =
    kind === 'short'
      ...
      : [
          ...
          '- Структура `settlements[]` включает подтвержденные поля карточки поселка: `name`, `short_name`, `slug`, `website`, `telegram`, `management_company`, полный `location`, полный `tariff`, `water_in_tariff`, `rabstvo`, `infrastructure`, `common_spaces`, `service_model`, `sources`, а также вычисленное поле `rating`.',
