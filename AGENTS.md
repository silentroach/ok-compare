...
- Если ожидаемый результат не супермелкий и важна точная строка/структура, предпочитай `toMatchInlineSnapshot()` вместо длинного `toBe(...)`/`toEqual(...)`, для большого вывода используй `toMatchSnapshot()`.
