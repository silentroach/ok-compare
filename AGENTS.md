...
- Публичный discovery по explorer feed завязан на `src/pages/for-agents.astro`, `src/pages/schemas/explorer.schema.json.ts`, `src/pages/openapi/explorer.openapi.json.ts`, `src/pages/.well-known/api-catalog.ts` и `src/lib/discovery.ts`; при изменениях `data/explorer.json`, его DTO, канонических URL, маршрута `/settlements/[slug]/` или описания рейтинга нужно синхронно обновлять эти файлы, чтобы agent docs, schema, OpenAPI и catalog не расходились с реальным payload.
