...
- Если ожидаемый результат не супермелкий и важна точная строка/структура, предпочитай `toMatchInlineSnapshot()` вместо длинного `toBe(...)`/`toEqual(...)`; для больших снапшотов (больше 5 строк) используй `toMatchSnapshot()`.
