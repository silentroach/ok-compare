...
ADR-004 включает `llms.txt` и `llms-full.txt` в миграцию, потому что эти `.txt` файлы фактически используют Markdown-структуру: заголовки, разделы, списки, ссылки и inline code.
