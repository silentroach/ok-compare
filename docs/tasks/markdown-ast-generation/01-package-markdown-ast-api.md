...
ADR-004 выбрал mdast и пакетный слой генерации. Текущий пакет уже содержит Markdown-to-HTML рендер, типографику и извлечение обычного текста. Новый слой должен быть рядом, но не должен ломать ADR-003 и рендер Markdown-тела в `apps/www`.
