# Task 024: Security Quote Item Table Fields

Status: [x] Completed

## Description

Enrich structured `source_refs[].quote_items` in `security.ts` with source-table columns from `security.pdf`: unit, quantity, unit price and total. Keep the legacy `quote` strings unchanged.

## Acceptance Criteria

- [x] Multi-position `quote_items` in `apps/www/src/data/reglament/estimate-details-2026/security.ts` include `quantity` with source unit where the PDF row has a quantity column.
...
- [x] Tests cover at least one enriched security equipment quote item.
...
- `apps/www/src/data/reglament/estimate-details-2026/security.ts`
- `apps/www/src/data/reglament/estimate-details-2026.test.ts`
- `docs/tasks/reglament-estimate-details/extra.md`

## Estimated Scope

Small: security has a focused equipment quote-item set.
