...
- [ ] Multi-position `quote_items` in `apps/www/src/data/reglament/estimate-details-2026/improvement.ts` include `quantity` with source unit where the PDF row has a quantity column.
...
- [ ] Tests cover at least one enriched improvement PPE/tool quote item.
