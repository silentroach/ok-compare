...
describe('estimate2026 baseline data', () => {
  ...
  it('keeps the disputed improvement repair source caveat explicit', () => {
    ...
    expect({
      ...
      {
        "annual_gross": 320424,
        "description": "Сопоставление строки итоговой сметы с детализацией не подтверждено: в final.pdf указано покрытие дорог и площадок, а в детализации благоустройства найдена только близкая по сумме строка ремонта периметрального ограждения. Официальная сумма сохранена по итоговой смете и требует проверки по исходной рабочей книге или у составителя сметы.",
        "materials": 298320,
        "source_refs": [
          {
            "fragment": "строка 4.2",
