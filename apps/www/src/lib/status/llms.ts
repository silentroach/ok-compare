...
export async function build(kind: 'short' | 'full'): Promise<string> {
  ...
  return kind === 'short'
    ? join([
        ...
        '',
        'Описание',
        '- Это статический раздел `/status` внутри kpshelkovo.online про текущие проблемы, плановые работы и историю по сервисам КП Шелково.',
        ...
        '- HTML-страницы остаются каноническим представлением для людей, а /status/data/status.json служит основным структурированным feed для агентов.',
        '',
        'Главные URL',
    ...
    : join([
        ...
        '',
        'Проект',
        '- Это статический раздел внутри kpshelkovo.online под `/status/...`.',
        '- Он нужен для публикации состояния сервисов КП Шелково, активных инцидентов, плановых работ и истории отключений/ограничений без SSR и mutation endpoints.',
        `- Сейчас в разделе ${count(data.incidents.length, ['запись', 'записи', 'записей'])}, ${count(activeIncidents.length, ['активный инцидент', 'активных инцидента', 'активных инцидентов'])} и ${count(activeMaintenance.length, ['активная работа', 'активные работы', 'активных работ'])}.`,
        '',
        'Канонические URL',
        `- Главная страница /status: ${home}`,
        ...
        '- Это основной read-only структурированный feed для агентов и машинного обхода раздела /status.',
        ...
        '',
        'Ограничения',
        '- Все маршруты /status генерируются статически; ручек для изменения данных и авторизации в этой поверхности нет.',
