...
export async function build(kind: 'short' | 'full'): Promise<string> {
  ...
  return kind === 'short'
    ? join([
        ...
        '- У новостей с календарным событием читайте `articles[].event.ics_url`; глобального календаря событий нет.',
    ...
    : join([
        ...
        '- События новостей представлены как optional `articles[].event`; `.ics` доступен по `/news/YYYY/MM/[entry]/event.ics`, глобального events feed нет.',
        ...
        '- Для новостей при массовом чтении используйте `news/data/articles.json`, а HTML/Markdown detail pages оставляйте для одной конкретной записи; если у статьи есть `event`, календарь берите из `event.ics_url`.',
...
export async function buildHomeMarkdown(): Promise<string> {
  ...
  return join([
    ...
    '',
    '## Разделы',
    `- [Новости](${absoluteUrl(newsUrl())}) — ${count(news.articles.length, ['статья', 'статьи', 'статей'])}; structured feed: ${absoluteUrl(articlesDataUrl())}; RSS: ${absoluteUrl(newsFeedUrl())}; события: optional articles[].event с article-local event.ics`,
