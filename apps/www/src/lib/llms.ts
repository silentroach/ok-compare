...
export async function build(kind: 'short' | 'full'): Promise<string> {
  ...
  return kind === 'short'
    ? join([
        ...
        '',
        'Описание',
        '- Это единый статический сайт kpshelkovo.online про поселок Шелково и связанные разделы.',
        '- Основные публичные разделы для агентов: новости, статус сервисов, регламентная смета, people-профили и сравнение поселков.',
        ...
        '- Public skill index на корневом сайте покрывает navigation, news, status и people; у compare-раздела есть свой skill index.',
      ])
    : join([
        'Шелково Онлайн',
        'Файл: llms-full.txt',
        ...
        '- В одном Astro-приложении живут home-страница и разделы `/news/`, `/status/`, `/815/regulation/`, `people` и `/815/compare/`.',
        '- Compare source находится в `apps/www/src/compare`, публичные маршруты — под `/815/compare/`.',
