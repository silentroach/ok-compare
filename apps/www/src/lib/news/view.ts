...
export const NEWS_PROSE =
  ...
  '[&_img]:rounded-[1.25rem] [&_img]:border [&_img]:border-border [&_img]:bg-[color:var(--color-surface-muted)] [&_img]:shadow-[var(--shadow-1)] ' +
  '[&_code]:rounded-md [&_code]:bg-[color:var(--color-surface-muted)] [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:text-[0.95em] ' +
  '[&_pre]:my-6 [&_pre]:overflow-x-auto [&_pre]:rounded-[1.25rem] [&_pre]:border [&_pre]:border-border [&_pre]:bg-[color:var(--color-surface-muted)] [&_pre]:p-4 [&_pre]:text-sm ' +
...
export function formatNewsMonth(
  year: number,
  month: number,
  opts?: { readonly capitalize?: boolean },
): string {
  const label = MONTH.format(new Date(Date.UTC(year, month - 1, 1)));
