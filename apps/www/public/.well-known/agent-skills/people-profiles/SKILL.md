...
description: Используй для чтения профилей людей через people.json и перехода к detail-страницам конкретных людей.
---
...
- Для people mentions используются `@slug`, `@slug:case` и `[видимый текст](@slug)`.
...
- Не используй `[текст](@slug:case)`: для labelled mention нужная грамматика должна быть написана в видимом тексте.
